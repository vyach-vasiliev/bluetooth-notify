...
Пункт «Настройки» доступен и в нижней части панели, и в нативном tray-меню. Страница открывается внутри той же панели и позволяет выбрать системный язык или один из девяти поддерживаемых: русский, английский, немецкий, японский, французский, испанский, португальский, китайский и корейский. Там же можно выбрать тему («Как в системе», светлая, тёмная), отдельно включить уведомления для среднего и низкого уровня заряда и задать их процентные пороги. Изменения применяются без перезапуска.
...
- `LocalizationService` — явный или системный выбор одной из девяти поддерживаемых UI-культур и локализованные `.resx`-ресурсы;
...
Секундный refresh работает только пока панель видима; при скрытой панели заряд проверяется раз в минуту для пороговых уведомлений. Изменения watcher-ов запускают отложенное обновление данных и статуса tray-иконки. Первый snapshot не создаёт уведомлений; переход `Disconnected → Connected` защищён debounce 3 секунды и повторно разрешается только после disconnect. Уведомление о заряде отправляется один раз при пересечении включённого порога сверху вниз.

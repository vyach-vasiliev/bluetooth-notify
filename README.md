...
Пункт «Настройки» доступен и в нижней части панели, и в нативном tray-меню. Страница открывается внутри той же панели и позволяет выбрать язык («Как в системе», English (United States), Русский), тему («Как в системе», светлая, тёмная), а также отдельно включить уведомления для среднего и низкого уровня заряда и задать их процентные пороги. Изменения применяются без перезапуска.
